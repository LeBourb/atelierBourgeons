...
class WC_Google_Merchant_Center_Feed extends WC_Integration {
	...
	public function tab_view() {
		...
		$active = get_post_meta( $post->ID, 'wc_gmcf_active', true );
                $product_description = $post->post_content;//$product_details['description'];
                //print_r($post);
		?>
		<div id="wc_gmcf_tab" class="panel woocommerce_options_panel">
			...
			<div id="wc_gmcf_items">
				<div id="wc_gmcf_tab_basic" class="options_group">
					...
					<?php
						// Description.
                                                /*$options['description'] = $product_description ;
						woocommerce_wp_textarea_input(
							array(
								'id' => 'wc_gmcf[description]',
								'label' => __( 'Description', 'wcgmcf' ),
								'description' => __( 'Description of the item', 'wcgmcf' ),
								'value' => $product_description//isset( $options['description'] ) ? $options['description'] : 'coco caline'
							)
						);*/
						// Category.
						...
						woocommerce_wp_select( array(
							...
							'value' => isset( $options['category'] ) ? $options['category'] : 'Robes',
                                                        'options' => array(
								'Robes',
                                                                'Hauts',
                                                                'Jupes',
						...
						woocommerce_wp_textarea_input( array(
							...
							'value' => $product_type //isset( $options['product_type'] ) ? $options['product_type'] : $this->product_type
						) );
				...
				<div id="wc_gmcf_tab_unique" class="options_group">
					...
					<div class="wc_ugp_wrap">
						<?php
							// Brand.
                                                        $brand = array_shift ( get_the_terms( $product->id, 'brand'));                                                        
					...
					<?php
						...
						woocommerce_wp_checkbox(
							array(
								...
								'value' => isset( $options['online_only'] ) ? $options['online_only'] : 'yes'
							)
						);
					?>
				</div>
	...
	public function feed_template( $page_template ) {
		if ( is_page( $this->page ) ) {
			$page_template = '../templates/feed.php';
		}
