...
if ( ! defined( 'ABSPATH' ) ) exit;
	...
	function amp_woocommerce_custom_woocommerce_template( $file, $type, $post ) {
		...
		if ( 'single' === $type && 'product' === $post->post_type ) {
			$file = dirname(__FILE__) . '/templates/amp-wc.php';
		}
		return $file;
	}
        ...
        /*add_filter( 'amp_post_template_customizer_settings','amp_post_template_customizer_settings_woocommerce_template' , 10, 3 );
        function amp_post_template_customizer_settings_woocommerce_template( $settings, $post ) {
		return 'test';
	}*/
...
*/
	...
	function amp_woocommerce_container_starts(){
        ...
        add_action( 'amp_post_template_metadata', 'amp_woocommerce_post_template_metadata', 9 );
        function amp_woocommerce_post_template_metadata($metadata){
               global $woocommerce;
               $product = $woocommerce->product_factory->get_product();
               $metadata['@type']='Product';
               $metadata['publisher']['logo']= esc_url( home_url( '/wp-content/themes/atelierbourgeons/icons/atelier_bourgeons.bmp' ) );
               $metadata['typeOfGood']='Product';
               $metadata['brand']='atelier Bourgeons';
               $metadata['manufacturer']='atelier Bourgeons';
               $metadata['material']=$product->get_attribute("size"); 
               $metadata['offers']['price']=$product->get_price(); 
               if( pll_current_language() == 'ja') 
                    $metadata['offers']['priceCurrency']='JPY'; 
               else 
               $metadata['offers']['priceCurrency']='EUR'; 
...
function amp_woocommerce_bfr_content() {
  // end of if condition for variant check
	...
	function amp_woocommerce_add_product_description(){
		...
		$sanitized_excerpt = new AMP_Content( $post_excerpt, array(), 
