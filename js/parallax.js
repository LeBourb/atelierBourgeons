$(function(){
	...
	$sections.each( function( ix ){
		...
		$sec.data( '$pNodes' , $sec.find( '.animate' ) );
                $sec.data( '$pMargin' , $sec.find( '.margin-img' ) );
	...
	function readCSSProps( $node, sClass ){
		...
		for( i=0, l=aAnimProps.length; i<l; i++ ){
			...
			switch( sProp ){
				...
				/*case 'top':
					if(sClass === "end")
                                            vPropVal = $node.position()[ sProp ] + $( window ).height();
                                        else
                                            vPropVal = $node.position()[ sProp ];
					break;
                                        */
	...
	function addDiffAnimation( $node, iTop, iStage, iAnimLength ){
		...
		oPropDiff = readCSSProps( $node, stages[ iStartStage ] );		
	...
	function measureAnimations(){
		...
		$sections.each( function( ix ){
			...
			iSecHeight = $sec[0].clientHeight;
                        iSecWidth = $sec[0].clientWidth;     
                        if(oData.$pNodes[0] != null){
                            var margin = ComputeMargin(oData.$pNodes[0].naturalWidth, oData.$pNodes[0].naturalHeight, iSecHeight, iSecWidth)
                        
                            /*oData.$pMargin.css('margin-left',margin.marginleft+'px');
                            oData.$pMargin.css('margin-right',margin.marginright+'px');
                            oData.$pMargin.css('margin-top',margin.margintop+'px');
                            oData.$pMargin.css('margin-bottom',margin.marginbottom+'px');
                            */
                            ...
                            //oData.$pNodes.css( hardwareCSSTransform( oCssProps ) );
                            
                        }
                        
                        
                        //addDiffAnimation( $sec, iTop, 1 );
			//}

			/*for( i=0, l=$pNodes.length; i<l; i++ ){
				$pNode = $pNodes.eq( i );
                                addDiffAnimation( $pNode, iTop , 1, iSecHeight );
                                addDiffAnimation( $pNode, iTop , 2, iSecHeight );
                                addDiffAnimation( $pNode, iTop , 3, iSecHeight );
				if( bAllowAnims ){
					iMaxPause = Math.max(
						  iMaxPause
						,             addDiffAnimation( $pNode, iTop                                     , 1, iSecHeight )
...
*/
                        for( i=0, l=$pNodes.length; i<l; i++ ){
				$pNode = $pNodes.eq( i );
                                $pNode.css({ visibility: 'visible', top: iTop});
			}
			$sec/*.detach()*/.css({ visibility: 'visible', top: iTop});
                        oData.iTop = iTop;
	...
	function partialCSSProp( iScrTop, oAnim, oProp ){
        ...
        function ComputeMargin(Width, Height, iSecheight, iSecwidth){
            ...
            if(Width > iSecwidth){
                margin.marginleft = -(Width - iSecwidth)/2;                                
            }else if(Height > iSecheight) {
                margin.margintop = -(Height - iSecheight)/2;
            }
            return margin;
        }
	...
	function onScrollHandler(){
		//return;
                ...
                var   cDate = +new Date()
		  , iScrTop = $content.offset().top - $window.scrollTop()
	...
	function onScroll( iScrTop ){
		var bChangedLoc = false
		  ...
		  ;
                  iSrcTop = iScrTop = $window.scrollTop() - $content.offset().top;
                var iSrcBottom = $window.scrollTop() + $window.height() - $content.offset().top;
                var iSrcMiddle =  ( iSrcBottom - iSrcTop )/ 2 + iSrcTop;
		
                //var parallaxtop = $content.offset().top;
		//iWinScrTop = iScrTop;

		//if( iScrTop < parallaxtop ){ iScrTop = parallaxtop; }
...
/*		for( i=0, l=$sections.length; i<l; i++ ){
			...
			if( ( oData.iTop <= iScrTop ) && ( oData.iBottom >= ( iScrTop ) ) ){
                ...
                */
                //iSecHeight = $sec[0].clientHeight;
                //iSecWidth = $sec[0].clientWidth;   
                // compute state of each section .... (start, focus, end)
                for( i=0, l=$sections.length; i<l; i++ ){
			$sec  = $sections.eq(i);
			oData = $sec.data();
                        iSecHeight = $sec[0].clientHeight;
                        iSecWidth = $sec[0].clientWidth;  
                        secmiddle = (oData.iBottom - oData.iTop)/2 + oData.iTop;
                        oDiff = - iSrcMiddle + secmiddle;
                        
                        
                        
                        //-iSrcMiddle
                        var visibility = '';
                        /*if( ( oData.iTop > iSrcBottom ) || ( oData.iBottom <  iScrTop  ) ) {
			...
			var    oData = $sec.data()
			  ,    $pNodes = oData.$pNodes;
                        ...
                        for( ii=0, ll=$pNodes.length; ii<ll; ii++ ){
                            $pNode = $pNodes.eq( ii );
                            //$pNode.removeClass("start").removeClass("end").removeClass("focus").addClass( sClass ).removeAttr( 'style' );
                            $pNode.css({ visibility: visibility});
                            //var maxmargintop = oData.iBottom - $pNode[0].scrollHeight;
                            var limittop = -0.9*$pNode[0].scrollHeight + oData.iTop;
                            var limitbottom = +0.9*$pNode[0].scrollHeight + oData.iTop;
                            
                            var img_left = ($pNode[0].clientWidth - $window.width())/2;
                            
                            var img_postop = 0;
                            pct = oData.iTop / (oData.iTop - limittop);
                            if(limittop > $window.scrollTop()){
                                img_postop = limittop;
                            }
                            else if (limitbottom < $window.scrollTop()) {
                                img_postop = limitbottom;
                            }else {
                                img_postop = 0.9 * ($window.scrollTop() - oData.iTop);
                            }
                            var maxmarginbottom = oData.iBottom - oData.iTop - $pNode[0].scrollHeight;
                            ...
                            */                           
                            ...
                            //var maxmarginbottom = window.getComputedStyle($pNode.parent()[0]).marginBottom.replace("px", "");
                            
                            //if(oDiff > 0 && oDiff > maxmargintop)
                            //    oDiff = maxmargintop;
                            //else if (oDiff < 0 && oDiff < maxmarginbottom ) 
                             //   oDiff = maxmarginbottom;
                            
                            //var height = oData.iBottom - oData.iTop;
                             
                            if(oDiff > 0)    
                                oDiff = 0;
                            else if (oDiff < maxmarginbottom)
                                oDiff = maxmarginbottom;
                            
                            oCssProps = {
                                top: margin.margintop,// img_postop,
                                left: margin.marginleft
                            };
                            /*oProps = oAnim.oProps;
                            for( n in oProps ){
				...
				//oCssProps[n] = 0|-( ( iScrTop - oProps[n][0] ) / ( oProps[n][1] - oProps[n][0] ) * ( oProps[n][1] - oProps[n][0] ) + oProps[n][0] );
                            ...
                            //$pNode.css( hardwareCSSTransform( oCssProps ) );
                        }
		...
		for( i=0, l=aAnimations.length; i<l; i++ ){
			...
			$node.css( hardwareCSSTransform( oCssProps , oAnim ) );
		}*/
	}
	...
	window.scrollToSection = function( sSec, immediate ){
		...
		var oData = $sect.data();
                ...
                if(!oData)
                    return;
		var top = oData.iTop + ( $sections[0] === $sect[0] ? 0 : iWindowHeight + 1 );
