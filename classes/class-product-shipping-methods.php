 ...
 * @author  WooThemes
 */
class WC_Shipping_AB_Free_Shipping extends WC_Shipping_Method {
    ...
    public function __construct( $instance_id = 0 ) {
        $this->id                 = 'free_shipping_atelierb';
        $this->instance_id        = absint( $instance_id );
        $this->method_title       = __( 'Livraison gratuite Atelier Bourgeons', 'atelierbourgeons' );
        $this->method_description = __( 'Cette méthode de livraison prend en compte les taux de conversion', 'atelierbourgeons' );
        $this->supports           = array(
            'shipping-zones',
            'instance-settings',
    ...
    public function is_available( $package ) {
        ...
        if ( in_array( $this->requires, array( 'min_amount', 'either', 'both' ) ) && isset( WC()->cart->cart_contents_total ) ) {
            ...
            if ( pll_current_language() == 'ja') {
                if ( $total >= EURToJPY($this->min_amount) ) 
...
function atelierb_shipping_method( $methods ) {
	$methods['free_shipping_atelierb'] = 'WC_Shipping_AB_Free_Shipping'; 
	return $methods;
}

add_filter( 'woocommerce_shipping_methods', 'atelierb_shipping_method' );